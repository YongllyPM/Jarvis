...
:: ── Si se pasa --install o --uninstall, modo silencioso (Inno Setup) ──────
...
if exist ".venv\Scripts\python.exe" (
    ".venv\Scripts\python.exe" install.py %1 %2
    exit
)

:: 2. Intentar buscar Python en la ruta estandar de instalacion del usuario (LocalAppData)
if exist "%LocalAppData%\Programs\Python\Python312\python.exe" (
    "%LocalAppData%\Programs\Python\Python312\python.exe" install.py %1 %2
    exit
)
if exist "%LocalAppData%\Programs\Python\Python313\python.exe" (
    "%LocalAppData%\Programs\Python\Python313\python.exe" install.py %1 %2
    exit
)
if exist "%LocalAppData%\Programs\Python\Python311\python.exe" (
    "%LocalAppData%\Programs\Python\Python311\python.exe" install.py %1 %2
    exit
)
...
if %errorlevel% equ 0 (
    python install.py %1 %2
    exit
)

:: 4. Intentar en Program Files por si acaso
if exist "%ProgramFiles%\Python312\python.exe" (
    "%ProgramFiles%\Python312\python.exe" install.py %1 %2
    exit
)
if exist "%ProgramFiles%\Python313\python.exe" (
    "%ProgramFiles%\Python313\python.exe" install.py %1 %2
    exit
)
